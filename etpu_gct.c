...
#include "etpu_util_ext.h"   /* General C Functions for the eTPU */
#include "etpu_set.h"        /* eTPU function set code binary image and other global ddefines */
#include "etpu_spi.h"        /* eTPU function SPI API */
...
/*******************************************************************************
 * eTPU channel settings - SPI_master 1
 ******************************************************************************/
struct spi_master_instance_t spi_master_1_instance =
{
    EM_AB,
    ETPU_SPI_MASTER1_SCLK_CHAN,
    { 0xff, 0xff, 0xff, 0xff, }, /* no slave selects */
    FS_ETPU_PRIORITY_MIDDLE,
    0,
    0,
};
struct spi_master_config_t spi_master_1_config =
{
    FS_ETPU_TCR1,
    0, /* CPOL */
    0, /* CPHA */
    FS_ETPU_SPI_LSB_FIRST,
    8,
    100000, /* 100kHz baud rate */
    20,
};

/*******************************************************************************
 * eTPU channel settings - SPI_slave 1
 ******************************************************************************/
struct spi_slave_instance_t spi_slave_1_instance =
{
    EM_AB,
    ETPU_SPI_SLAVE1_SCLK_CHAN,
    0xff, /* no ss input for this instance */
    FS_ETPU_PRIORITY_MIDDLE,
    0,
    0,
};
struct spi_slave_config_t spi_slave_1_config =
{
    FS_ETPU_TCR1,
    0, /* CPOL */
    0, /* CPHA */
...
int32_t my_system_etpu_init(void)
{
  ...
  ...
    ...
    if(err_code != FS_ETPU_ERROR_NONE) return(err_code + (ETPU_SPI_MASTER1_SCLK_CHAN<<16));

    err_code = fs_etpu_spi_slave_init(&spi_slave_1_instance, &spi_slave_1_config);
    if(err_code != FS_ETPU_ERROR_NONE) return(err_code + (ETPU_SPI_SLAVE1_SCLK_CHAN<<16));
